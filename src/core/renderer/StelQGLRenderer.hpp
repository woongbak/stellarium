...
public:
    ...
    virtual void drawEllipse(const float x, const float y,
                             const float width, const float height,
                             const float angle = 0.0f);
...
private:
    ...
    //! Vertex buffer used to draw plain, non-textured ellipses.
    StelVertexBuffer<VertexP2>* plainEllipseBuffer;
    ...
    //! Implements drawEllipse().
    //!
    //! These are pretty much the same function, but with different names to improve
    //! readability.
    //!
    //! @param renderer Renderer that's drawing the ellipse.
    //! @param textured Should the rectangle be textured?
    //! @param x        Horizontal position of the ellipse.
    //! @param y        Vertical position of the ellipse.
    //! @param r1       Width of the ellipse.
    //! @param r2       Height of the ellipse.
    //! @param PA       Rotation (Principal) angle of the ellipse in degrees.
    //!
    //! @see drawEllipse
    void drawEllipseInternal(const bool textured, const float x, const float y,
                          const float r1, const float r2, const float PA);
