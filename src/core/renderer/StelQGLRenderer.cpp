...
void StelQGLRenderer::drawEllipseInternal
    (const bool textured, const float x, const float y, const float r1,
     const float r2, const float PA)
{
    //statistics[RECT_DRAWS] += 1.0;
    // Could be improved by keeping the vertex buffer as a data member,
    // or even caching all rectangle draws to the same buffer and drawing them
    ...
    {
        const float cosr       = std::cos(PA / 180 * M_PI);
        const float sinr       = std::sin(PA / 180 * M_PI);
        const float halfWidth  = r1 * 0.5;
        const float halfHeight = r2 * 0.5;
    ...
    if(NULL == plainEllipseBuffer)
    {
        plainEllipseBuffer =
            createVertexBuffer<VertexP2>(PrimitiveType_LineLoop);
    }
    else
    {
        plainEllipseBuffer->unlock();
        plainEllipseBuffer->clear();
    }

    for (int i=0;i<segments;i++)
    {
        Vec2f   pos = Vec2f(x + xp*cosPA - yp*sinPA, y + xp*sinPA + yp*cosPA);
        plainEllipseBuffer->addVertex(pos);
    ...
    plainEllipseBuffer->lock();
    drawVertexBuffer(plainEllipseBuffer);
}
...
void StelQGLRenderer::drawEllipse(const float x, const float y,
                                  const float width, const float height,
                                  const float angle)
{
    drawEllipseInternal(false, x, y, width, height, angle);
}
