...
#ifndef _TELESCOPE_CLIENT_DIRECT_SERVOCAT_
#define _TELESCOPE_CLIENT_DIRECT_SERVOCAT_
...
class ServoCatConnection;

//! Telescope client that connects directly to a StellarCAT ServoCAT through
//! a serial port.
//! This class has been based on TelescopeClientDirectServoCAT.
class TelescopeClientDirectServoCat : public TelescopeClient, public Server
{
	Q_OBJECT
public:
	TelescopeClientDirectServoCat(const QString &name, const QString &parameters, Equinox eq = EquinoxJ2000);
	~TelescopeClientDirectServoCat(void)
	{
		//hangup();
	}
...
private:
	...
	ServoCatConnection *servoCat;
	
	bool queue_get_position;
	long long int next_pos_time;
};

#endif //_TELESCOPE_CLIENT_DIRECT_SERVOCAT_
